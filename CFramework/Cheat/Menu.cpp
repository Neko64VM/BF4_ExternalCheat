...
void CFramework::RenderMenu()
{
    ...
    ImGui::Begin("Battlefield 4 [ EXTERNAL ]", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse);
    ...
    case 0:
        ...
        break;
    case 1: // visual
        ImGui::BeginChild("##C110", ImVec2(ImGui::GetContentRegionAvail().x, ImGui::GetContentRegionAvail().y / 4.f), true);

        ImGui::Text("ESP Setting");
        ImGui::Separator();
        ImGui::Spacing();

        ImGui::CustomSliderInt("Distance", "##Distance", &g.ESP_MaxDistance, 25, 2000);
        ...
        ImGui::ColorEdit3("EnemyVehicle", &g.Color_ESP_EnemyVehicle.Value.x);
        ImGui::ColorEdit3("Team", &g.Color_ESP_Team.Value.x);
        ImGui::ColorEdit3("TeamVehicle", &g.Color_ESP_TeamVehicle.Value.x);
        // ImGui::ColorEdit3("AimingTarget", &g.Color_ESP_AimTarget.Value.x);
        ...
        if (g.GenerateFlag)
        {
            ...
            if (ImGui::Button("Generate", ImVec2(ImGui::GetContentRegionAvail().x / 2.f, 20.f)) || utils::IsKeyDown(VK_RETURN))
            {
                if (g.newConfigName.size() > 1) {
                    std::thread([&]() { config.CreateConfig(g.newConfigName); }).join();
                }
